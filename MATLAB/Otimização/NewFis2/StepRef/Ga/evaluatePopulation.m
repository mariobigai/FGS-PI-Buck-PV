function [fitness, cont_simu] = evaluatePopulation(parameters,population)
   ...
   for j=1:size(population,1)
       ...
       %Penalizações com base nas restrições --------------------------------------------------------
        ...
        if (any(population(j,1:2) > parameters.Restr(2)) || any(population(j,1:2) < parameters.Restr(1)));fitness(j,1) = 500; continue; end
        if (any(population(j,3:4) > parameters.Restr(4)) || any(population(j,3:4) < parameters.Restr(3)));fitness(j,1) = 500; continue; end
        if (any(population(j,5:20) > parameters.Restr(6)) || any(population(j,5:20) < parameters.Restr(5)));fitness(j,1) = 500; continue; end
        if (any(population(j,21:70) > parameters.Restr(8)) || any(population(j,21:70) < parameters.Restr(7)));fitness(j,1) = 500; continue; end
        if (any(population(j,71:end) > parameters.Restr(10)) || any(population(j,71:end) < parameters.Restr(9)));fitness(j,1) = 500; continue; end
...
%Estabelece os inputs da simulação
                simIn = Simulink.SimulationInput('buck_pv_otm_step_vref2');
