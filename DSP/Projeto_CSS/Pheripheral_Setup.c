...
void Setup_GPIO(void){
    ...
    GpioCtrlRegs.GPAMUX1.bit.GPIO2 = 1;                 // Pino 2 configurado como EPWM2A

    GpioCtrlRegs.GPAGMUX1.bit.GPIO3 = 0;
    GpioCtrlRegs.GPAMUX1.bit.GPIO3 = 1;                 // Pino 3 configurado como EPWM2B
...
void Setup_ePWM(void){
    ...
    CpuSysRegs.PCLKCR2.bit.EPWM2 = 1;                   // Habilita o clock do Módulo de PWM n° 2
    ...
    //PWM2A e PWM2B (PINOS P2 e P3)- EmuladoPV
    EPwm2Regs.TBPRD = 2000;                  // Configura o período do PWM 2 (fs=50kHz)

    EPwm2Regs.TBPHS.bit.TBPHS = 0;// Configura o deslocamento de phase (sem deslocamento)
    EPwm2Regs.TBCTL.bit.PRDLD = TB_SHADOW;
    EPwm2Regs.TBCTL.bit.SYNCOSEL = TB_CTR_ZERO;
    EPwm2Regs.TBCTR = 0;

    EPwm2Regs.TBCTL.bit.CTRMODE = TB_COUNT_UP;          // Configura o modo de contagem como crescente
    EPwm2Regs.TBCTL.bit.PHSEN = TB_DISABLE;              // Desabilita o deslocamento de fase
    EPwm2Regs.TBCTL.bit.HSPCLKDIV = TB_DIV1;            // Clock (prescaler) alta velocidade
    EPwm2Regs.TBCTL.bit.CLKDIV = TB_DIV1;               // Clock (prescaler)

    EPwm2Regs.CMPCTL.bit.SHDWAMODE = CC_SHADOW;         // Habilita o SHADOW
    EPwm2Regs.CMPCTL.bit.LOADAMODE = CC_CTR_ZERO_PRD;   // Carrega quando atingir zero ou o PRD
    EPwm2Regs.CMPCTL.bit.SHDWBMODE = CC_SHADOW;         // Habilita o SHADOW
    EPwm2Regs.CMPCTL.bit.LOADBMODE = CC_CTR_ZERO_PRD;   // Carrega quando atingir zero ou o PRD

    EPwm2Regs.AQCTLA.bit.PRD = AQ_NO_ACTION;            // Nenhuma ação ao atingir PRD
    EPwm2Regs.AQCTLA.bit.ZRO = AQ_SET;                  // Set ao atingir zero
    EPwm2Regs.AQCTLA.bit.CAU = AQ_CLEAR;                // Reset ao atingir CMPA na subida
    EPwm2Regs.AQCTLA.bit.CAD = AQ_NO_ACTION;            // Nenhuma ação ao atingir CMPA na descida

    EPwm2Regs.DBCTL.bit.POLSEL = DB_ACTV_HIC;           // Habilita o pulso complementar
    EPwm2Regs.DBCTL.bit.OUT_MODE = DB_FULL_ENABLE;      // Habilita o Módulo de Tempo Morto
    EPwm2Regs.DBRED.bit.DBRED = 0.015*2000;             // Tempo Morto
...
void Setup_ADC(void){
    ...
    AdcaRegs.ADCSOC3CTL.bit.CHSEL = 2;                  // Selação do ADC a ser utilizado
    ...
    AdcaRegs.ADCSOC6CTL.bit.CHSEL = 1;                  // Selação do ADC a ser utilizado
    ...
    AdcaRegs.ADCSOC7CTL.bit.CHSEL = 1;                  // Selação do ADC a ser utilizado
    ...
    AdcaRegs.ADCSOC9CTL.bit.CHSEL = 4;                  // Selação do ADC a ser utilizado
    ...
    AdcaRegs.ADCSOC10CTL.bit.CHSEL = 4;                  // Selação do ADC a ser utilizado
    ...
    AdcaRegs.ADCSOC11CTL.bit.CHSEL = 4;                  // Selação do ADC a ser utilizado
    ...
    AdcaRegs.ADCSOC12CTL.bit.CHSEL = 0;                  // Selação do ADC a ser utilizado
    AdcaRegs.ADCSOC12CTL.bit.ACQPS = acqps;              // Tamanho da janela de amostragem
    AdcaRegs.ADCSOC12CTL.bit.TRIGSEL = 5;                // Confifuração do evento que dispara o ADC (PWM2 SOCA/C)
    AdcaRegs.ADCSOC13CTL.bit.CHSEL = 0;                  // Selação do ADC a ser utilizado
    ...
    AdcaRegs.ADCSOC14CTL.bit.CHSEL = 5;                  // Selação do ADC a ser utilizado
    ...
    AdcaRegs.ADCSOC15CTL.bit.CHSEL = 5;                  // Selação do ADC a ser utilizado
